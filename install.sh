...
function configure_qbittorrent_nox(){
    if [[ $Root == 1 ]]; then
        ...
        bash <(wget -qO- https://github.com/Aniverse/inexistence/raw/master/00.Installation/package/qbittorrent/configure -o /dev/null) -u $iUser -p $iPass
        echo -e "http://$serverip:2017"
    else
